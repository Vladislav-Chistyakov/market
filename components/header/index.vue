...
const isWishlistPage = computed(() => {
  return route.path.includes('wishlist')
})

const isMyAccountPage = computed(() => {
  return route.path.includes('my-account') && !route.path.includes('wishlist')
})


const isCartPage = computed(() => {
  return route.path.includes('placing-an-order') || route.path.includes('cart')
})
</script>
  ...
  >
    ...
    >
      ...
      <input
        disabled
        type="text"
        class="outline-none placeholder-gray-200 max-w-[120px] bg-gray-50 rounded-[8px] font-causten font-medium text-[18px] leading-[22px] text-gray-200"
        placeholder="Search"
      />
    </label>
    ...
    <ul class="flex flex-row items-center gap-3">
      <li>
        <nuxt-link
          ...
          class="h-[44px] w-[44px] rounded-[8px] p-1 flex items-center justify-center"
          :class="[isWishlistPage ? 'bg-purple' : 'bg-gray-50']"
          aria-label="Wishlist"
        >
          <svg
            class="flex"
          ...
          >
            <path
              ...
              :stroke="isWishlistPage ? '#ffffff' : '#807D7E'"
              stroke-width="1.5"
      ...
      <li>
        <nuxt-link
          ...
          class="h-[44px] w-[44px] rounded-[8px] p-1 flex items-center justify-center"
          :class="[isMyAccountPage ? 'bg-purple' : 'bg-gray-50']"
          aria-label="User"
        >
          <svg
            width="16"
          ...
          >
            <path
              d="M8.00016 9.66668C10.3013 9.66668 12.1668 7.8012 12.1668 5.50001C12.1668 3.19882 10.3013 1.33334 8.00016 1.33334C5.69898 1.33334 3.8335 3.19882 3.8335 5.50001C3.8335 7.8012 5.69898 9.66668 8.00016 9.66668ZM8.00016 9.66668C4.31826 9.66668 1.3335 11.9053 1.3335 14.6667M8.00016 9.66668C11.6821 9.66668 14.6668 11.9053 14.6668 14.6667"
              :stroke="isMyAccountPage ? '#ffffff' : '#807D7E'"
              stroke-width="1.5"
              stroke-linecap="round"
            />
          </svg>
      ...
      <li>
        <nuxt-link
          to="/cart"
          class="h-[44px] w-[44px] rounded-[8px] p-1 flex items-center justify-center"
          :class="[isCartPage ? 'bg-purple' : 'bg-gray-50']"
          aria-label="Basket"
        >
          <svg
            width="17"
          ...
          >
            <path
              ...
              :stroke="isCartPage ? '#ffffff' : '#807D7E'"
              stroke-width="1.5"
              stroke-linecap="round"
            />
          </svg>
